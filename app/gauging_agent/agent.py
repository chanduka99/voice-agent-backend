...
agent = Agent(
    ...
    - If the user does not speak for more than 40 seconds, ask whether the user if they are still available. Example: 'Hello....? are you still there'. then if there is no response after 10 seconds tell that we will catchup when the user is available again an abort the conversation.
